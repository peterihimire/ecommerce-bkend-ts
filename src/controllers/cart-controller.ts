...
// @desc Login into account
// @access Private
export const addCartQty: RequestHandler = async (req, res, next) => {
...
export const updateCartProdQty: RequestHandler = async (req, res, next) => {
  const { passport } = req.session;
  const email = passport?.user;
  ...
  try {
    ...
    const cart = await foundUserCartId(existing_user.id);
    const prod_info = await foundProductId(prod_id);
    let cartProduct = await foundCartProd(cart.id, prod_info.id);
    ...
    // Recalculate total price and quantity of the cart
    const cart_prods = await foundCartId(existing_user.cart.id);
    const totalCartPrice = cart_prods.products.reduce(
    ...
    const totalCartQty = cart_prods.products.reduce(
      (total: number, item: any) => {
        return total + item.cart_products.quantity;
      },
      0
